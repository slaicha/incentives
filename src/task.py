...
def train(net, trainloader, epochs, lr, device):
    ...
    for _ in range(epochs):
        for batch in trainloader:
            images = batch["image"]
            # images = batch["img"] # for CIFAR10
            labels = batch["label"]
            optimizer.zero_grad()
            loss = criterion(net(images.to(device)), labels.to(device))
            loss.backward()
            torch.nn.utils.clip_grad_norm_(net.parameters(), max_norm=1.0) # added clipping
            optimizer.step()
            running_loss += loss.item()
            
    # avg_trainloss = running_loss / len(trainloader) # concern about how the loss is calculated
    avg_trainloss = running_loss / (epochs * len(trainloader)) # I think this makes more sense
    
    return avg_trainloss


def pretrain(net, trainloader, epochs, lr, device):
    """Pretrain the model locally and estimate G_n^2."""
    net.to(device)  # move model to GPU if available
    criterion = torch.nn.CrossEntropyLoss().to(device)
    optimizer = torch.optim.SGD(net.parameters(), lr=lr, momentum=0.9)
    net.train()
    ...
    for _ in range(epochs):
        ...
        for batch in trainloader:
            ...
            # images = batch["img"].to(device) # for CIFAR10
    ...
    Gn_squared = sum(squared_norms) / len(squared_norms) if squared_norms else 0.0
    avg_trainloss = running_loss / len(trainloader)

    return Gn_squared, avg_trainloss
